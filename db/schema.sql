...
CREATE TRIGGER IF NOT EXISTS client_ai AFTER INSERT ON client BEGIN
INSERT INTO
  client_fts (id, first_name, middle_name, last_name)
VALUES
  (
    new.id,
    new.first_name,
...
CREATE TRIGGER IF NOT EXISTS client_au AFTER
...
CREATE TRIGGER IF NOT EXISTS client_ad AFTER DELETE ON client BEGIN
DELETE FROM client_fts
WHERE
  id = old.id;
